...
def optionemtricsdata_transform(spxoptions_df, startdate, enddate):
    ...
    if spxspot_df.empty:
        print("Download failed or rate-limited. Try again later or reduce request frequency.")
        return
    ...
    for i in range(len(observation_dates)):
        match = spxspot_df.loc[spxspot_df["date_dt"] == observation_dates[i], "Close"]
        if not match.empty:
            spot[i] = match.iloc[0]
        else:
            # Fallback: use nearest available date (important if markets differ slightly)
            nearest_idx = (spxspot_df["date_dt"] - observation_dates[i]).abs().idxmin()
            spot[i] = spxspot_df.loc[nearest_idx, "Close"]
