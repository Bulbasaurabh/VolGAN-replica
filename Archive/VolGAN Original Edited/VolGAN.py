...
def arbitrage_penalty_tensor(C, P_T, P_K, PB_K, lk=10, lt=8):
    """
    Arbitrage penalty for flattened surfaces.
    C: [batch_size, lk*lt] flattened surfaces
    P_T, P_K, PB_K: penalty matrices of shapes [lt, lt] and [lk, lk]
    """
    batch_size = C.shape[0]
    device = C.device

    # Reshape from [batch_size, lk*lt] → [batch_size, lk, lt]
    C = C.view(batch_size, lk, lt)

    # Penalty for monotonicity in T (increasing)
    P1 = torch.matmul(C, P_T.to(device))

    # Penalty for monotonicity in K (decreasing)
    P2 = torch.matmul(P_K.to(device), C)

    # Penalty for convexity in K
    P3 = torch.matmul(PB_K.to(device), C)

    # Clamp negative values (violations)
    P1 = torch.clamp(P1, min=0)
    P2 = torch.clamp(P2, min=0)
    P3 = torch.clamp(P3, min=0)

    # Total penalty per batch
    total = torch.sum(P1, dim=(1, 2)) + torch.sum(P2, dim=(1, 2)) + torch.sum(P3, dim=(1, 2))
...
def SPXData(datapath, surfacespath):
    """
    Read pre-processed SPX implied vol data and SPY prices, compute log returns, and prepare grids.
    """
    # --- Read CSV files ---
    data = pd.read_csv(datapath)
    sfts_df = pd.read_csv(surfacespath)
    surfaces_transform = np.array(sfts_df)[:, 1:]  # skip first column

    # --- Dates ---
    dates_dt = pd.to_datetime(data['date'], errors='coerce')  # infer format, ignore bad values
    dates_dt = dates_dt.dropna().unique()  # remove any NaT

    # --- Days to expiry ---
    data_temp = data[data['date'] == data['date'].iloc[0]]
    days = pd.to_numeric(data_temp['days'], errors='coerce')
    days = days.dropna().unique()

    # --- SPY prices ---
    y_symbols = 'SPY'
    startdate = datetime(2000,1,3)
    enddate = datetime(2023,8,31)
    SPY = yf.download(y_symbols, start=startdate, end=enddate, progress=False)
    prices = np.array(SPY['Close'] * 10)

    # --- Previous-day prices ---
    prev_day = datetime(1999,12,31)
    SPY_prev = yf.download(y_symbols, start=prev_day, end=startdate, progress=False)
    if SPY_prev.empty:
        prev_price = prices[0]  # fallback
    else:
        prev_price = SPY_prev['Close'].iloc[0] * 10

    prices_prev = np.roll(prices, 1)
    prices_prev[0] = prev_price

    # --- Log returns ---
    log_rtn = np.log(prices) - np.log(prices_prev)


    # pick the first 8 maturities after the first
    tau_values = days[:8]  # could be fewer than 8 if data short
    if len(tau_values) == 0:
        raise ValueError("No valid maturities found in 'days' column. Check your CSV.")

    tau = tau_values / 365  # convert to years

    dtm = days[:8]
    # print("tau_values:", tau_values)
    # print("Grid shape -> m:", len(m), "tau:", len(tau_values), "total:", len(m)*len(tau_values))
...
def DataPreprocesssing(datapath, surfacepath):
    ...
    log_iv_tm1 = log_iv_tm1.reshape((log_iv_tm1.shape[0], -1))  # shape (N, m_dim*k_dim)
    log_iv_inc_t = log_iv_t - np.log(surfaces_transform[21:-1])
    log_iv_inc_t = log_iv_inc_t.reshape((log_iv_inc_t.shape[0], -1))  # shape (N, m_dim*k_dim)
    ...
    if realised_vol_tm1.ndim == 1:
        realised_vol_tm1 = realised_vol_tm1[:, np.newaxis]
    #condition for generator and discriminator
    log_iv_tm1 = log_iv_tm1[:5931, :]
    condition = np.concatenate((
        log_rtn_tm1,         # (5931, 1)
        log_rtn_tm2,         # (5931, 1)
        realised_vol_tm1,    # (5931, 1)
        log_iv_tm1           # (5931, 30)
    ), axis=1)               # result = (5931, 33)

    #true: what we are trying to predict, increments at time t
    log_rtn_t_ann = np.sqrt(252) * log_rtn_t
    log_iv_inc_t = log_iv_inc_t[:5931, :]
    true = np.concatenate((log_rtn_t_ann,log_iv_inc_t),axis=1)
...
def GradientMatching(gen,gen_opt,disc,disc_opt,criterion,condition_train,true_train,m,tau,ms,taus,n_grad,lrg,lrd,batch_size,noise_dim,device, lk = 10, lt = 8):
    ...
    tau_t, _ = torch.sort(tau_t)
    Ngrid = lk * lt
    eps = 1e-2
    t_seq = torch.zeros((tau_t.shape[0]-1), dtype=torch.float, device=device)
    for i in range(tau_t.shape[0]-1):
        diff = tau_t[i+1] - tau_t[i]
        diff_clamped = torch.clamp(diff, min=eps)
        t_seq[i] = 1 / (diff_clamped ** 2)
    # t_seq[~torch.isfinite(t_seq)] = 0.0
    matrix_t = torch.zeros((Ngrid,Ngrid), device = device, dtype = torch.float)
    for i in range(Ngrid-1):
        matrix_t[i,i] = -1
        matrix_t[i,i+1] = 1
    tsq = torch.zeros((1, Ngrid), dtype=torch.float, device=device)
    tau_indices = torch.linspace(0, len(tau_t)-1, Ngrid).long()  # maps 80 rows to 8 tau points
    for i in range(Ngrid - 1):
        diff = (tau_t[tau_indices[i + 1]] - tau_t[tau_indices[i]]).abs()
        diff_clamped = torch.clamp(diff, min=eps)
        tsq[0, i] = 1 / (diff_clamped ** 2)
    tsq[0, Ngrid - 1] = tsq[0, Ngrid - 2]
    ...
    # print("tau_t", tau_t, tau_t.shape)
    # print("t_seq", t_seq, t_seq.shape)
    # print("tsq", tsq, tsq.shape)
    n_epochs = n_grad
    ...
    t_smooth_grad = []
    gen.train()
    # print("DEBUG: lk =", lk, "lt =", lt)
    # print("DEBUG: Ngrid =", Ngrid)
    # print("DEBUG: fake surface shape:", fake_surface.shape)
    # print("DEBUG: tsq shape:", tsq.shape)
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(n_batches):
            ...
            # print(fake_surface.shape)  # probably (n_samples, 30)
            n_samples = fake_surface.shape[0]
            fake_surface_padded = fake_surface  # now shape (n_samples, 80)

            penalties_m = torch.zeros(n_samples, device=fake_surface.device)
            penalties_t = torch.zeros(n_samples, device=fake_surface.device)
            # print("penalties m", penalties_m.shape) 
            # print("penalties t", penalties_t.shape) 
            # print("matrix m", matrix_m.shape) 
            # print("matrix t", matrix_t.shape) 
            # print("tsq", tsq.shape)
            # print("fake surface", fake_surface.shape) 
            # print("fake surface padded", fake_surface_padded.shape) 
            
            for iii in range(curr_batch_size):
                penalties_m[iii] = torch.matmul(m_seq,(torch.matmul(matrix_m,fake_surface_padded[iii])**2))
                penalties_t[iii] = torch.matmul(tsq,(torch.matmul(matrix_t,fake_surface_padded[iii])**2))
    ...
    # print("m_smooth_grad mean:", np.mean(m_smooth_grad))
    # print("t_smooth_grad mean:", np.mean(t_smooth_grad))
    # print("Any zeros in t_smooth_grad?", np.any(np.array(t_smooth_grad) == 0))
    # print("Any NaNs in t_smooth_grad?", np.isnan(np.array(t_smooth_grad)).sum())
    alpha = np.mean(np.array(BCE_grad) / np.array(m_smooth_grad))
    t_smooth_grad_clean = np.nan_to_num(t_smooth_grad, nan=1e-8)
    beta = np.mean(np.array(BCE_grad) / np.array(t_smooth_grad_clean))
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(n_batches):
            ...
            for iii in range(curr_batch_size):
                ...
                val = torch.matmul(tsq, (torch.matmul(matrix_t, fake_surface[iii]) ** 2))
                penalties_t[iii] = val
                if torch.isnan(val) or torch.isinf(val):
                    print(f"NaN or Inf in penalties_t at index {iii}, value: {val}")
            m_penalty = sum(penalties_m) / curr_batch_size
            t_penalty = sum(penalties_t) / curr_batch_size
...
def TrainLoopNoVal(alpha,beta,gen,gen_opt,disc,disc_opt,criterion,condition_train,true_train,m,tau,ms,taus,n_epochs,lrg,lrd,batch_size,noise_dim,device, lk = 10, lt = 8):
    ...
    tsq = t_seq.repeat(lk).unsqueeze(0)
    expected_len = lk * lt
    if tsq.shape[1] < expected_len:
        pad_len = expected_len - tsq.shape[1]
        last_val = tsq[0, -1].item()
        pad_tensor = torch.full((1, pad_len), last_val, device=device, dtype=tsq.dtype)
        tsq = torch.cat([tsq, pad_tensor], dim=1)
    matrix_m = torch.zeros((Ngrid-lk,Ngrid), device = device, dtype = torch.float)
    for i in range(Ngrid-lk):
        matrix_m[i,i] = -1
        matrix_m[i,i+lk] = 1
    ...
    for epoch in tqdm(range(50)):
        ...
        for i in range(n_batches):
            ...
            penalties_t = [None] * curr_batch_size
            n_samples = fake_surface.shape[0]
            fake_surface_padded = fake_surface  # now shape (n_samples, 80)  # keep your 30 features
            # print("matrix m", matrix_m.shape) 
            # print("matrix t", matrix_t.shape) 
            # print("tsq", tsq.shape)
            # print("fake surface", fake_surface.shape) 
...
def VolGAN(datapath,surfacepath, tr, noise_dim = 16, hidden_dim = 8, n_epochs = 1000,n_grad = 100, lrg = 0.0001, lrd = 0.0001, batch_size = 100, device = 'cpu'):
    ...
    print("true_train.shape:", true_train.shape)
    print("condition_train.shape:", condition_train.shape)
